defmodule CcWeb.RealmsLive.Components.Profile do
  ...
  def render(assigns) do
    temple do
      div class: ~u"flex flex-col flex-shrink-0 w-1/4 max-w-xs bg-white shadow-xl" do
        div class: ~u"flex items-center h-16 px-4 shadow-md" do
          div do
            h2 class: ~u"text-lg font-bold text-gray-800" do
              "Profile"
            end
          end

          button "phx-click": "close-profile",
                 class: ~u"w-6 h-6 rounded hover:bg-gray-300 ml-auto
        ...
        div class: ~u"flex flex-col flex-grow overflow-auto p-4" do
          div class: ~u"mb-4" do
            if @current_user.id != @user.id do
              c &user_avatar/1, class: ~u"w-48 rounded mx-auto", user: @user
            else
              c &form/1,
                for: %{},
              ...
              do
                div class: ~u"mb-4" do
                  if Enum.any?(@uploads.avatar.entries) do
                    div class: ~u"mx-auto mb-2 w-48" do
                      ...
                      button type: "submit",
                             class: ~u"w-full bg-emerald-600 hover:bg-emerald-700
                    ...
                    c &user_avatar/1, user: @user, class: ~u"w-48 rounded mx-auto"
                  end
                end

                label class: ~u"block mb-2 font-semibold text-lg text-gray-800" do
                  "Upload avatar"
                end

                c &live_file_input/1, class: ~u"w-full", upload: @uploads.avatar
              end
          ...
          h2 class: ~u"text-xl font-bold text-gray-800" do
            @user.username
          end
        end
      end
